...
export default function Home() {
  ...
  const handleUploaded = () => {
    // Invalidate the feed query so the new photo appears
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 w-full">
        ...
        {!session?.user && (
          <section className="mx-auto max-w-3xl px-4 sm:px-6 py-16 sm:py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground/70 mb-10">
              Features 1-3/12: Authentication · Upload · Feed
            </p>
        ...
        {/* Feed — always visible (anonymous users can browse) */}
        <FeedView />
        ...
        {session?.user && (
          <div className="mx-auto max-w-7xl px-4 sm:px-6 pb-8 -mt-4">
            <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                Welcome back, {session.user.name || session.user.username} — features 4-12 coming soon
              </span>
            </div>
          </div>
        )}
